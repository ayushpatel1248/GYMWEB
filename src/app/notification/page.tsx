...
const page = () => {
  ...
  const generateSuscribeEndPoint = async (
  ...
  const subscribeUser = async () => {
    if ("serviceWorker" in navigator) {
      try {
        ...
        console.error(
          "Error in service worker registration or subscription:",
          error
        );
      }
    } else {
  ...
  return (
    <>
      <div onClick={showNotification}>page</div>
      <div className="pt-6">
        <button
          onClick={async () => {
            await sendNotification(
              "notification you site is hacked",
              "14",
              "",
              "message"
            );
